...
`@codesandbox/sandpack-react` ships a static stylesheet generated at build time with [vanilla-extract](https://vanilla-extract.style). The styles can be loaded via `import "@codesandbox/sandpack-react/styles.css"` or inlined for SSR with `getSandpackCssText()`. If you want to ship Sandpack components without any styling so you can apply your own CSS, we also provide an unstyled build that returns empty class names for every component.

To do it, you need to consume the same components from the `unstyled` subfolder, which doesn't include the bundled CSS. For example: 
