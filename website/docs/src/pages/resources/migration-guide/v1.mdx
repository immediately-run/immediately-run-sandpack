...
Sandpack ships its styles as a static stylesheet generated at build time with [vanilla-extract](https://vanilla-extract.style). For SSR you can either import the bundled stylesheet directly with `import "@codesandbox/sandpack-react/styles.css"`, or inline the CSS string returned by `getSandpackCssText()`:
